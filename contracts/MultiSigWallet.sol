...
contract MultiSigWallet is Ownable, IERC721Receiver, ERC1155Receiver {
    ...
    mapping(address => mapping(address => bool))
        public signerChangeConfirmations;
    ...
    modifier confirmed(uint transactionId, address signer) {
        require(
            confirmations[transactionId][signer],
            "Transaction is not confirmed by the signer"
        );
        _;
    }
    ...
    modifier validRequirement(uint _signerCount, uint _required) {
        require(
            _signerCount <= MAX_OWNER_COUNT &&
                required <= _signerCount &&
                _required != 0 &&
                _signerCount != 0,
            "Invalid requirement"
        );
    ...
    function confirmSignerChange(
        address _oldSigner,
        address _newSigner
    ) external signerExists(msg.sender) {
        require(
            !signerChangeConfirmations[_newSigner][msg.sender],
    ...
    function submitTransaction(
        ...
        bytes memory data,
        uint confirmTimestamp
    ) public signerExists(msg.sender) returns (uint transactionId) {
    ...
    {
        require(
            !isTransactionTimedOut(transactionId),
    ...
    function isConfirmed(uint transactionId) public view returns (bool) {
        ...
        mapping(address => bool)
            storage transactionConfirmations = confirmations[transactionId];

        uint256 signerCount = signers.length;
        for (uint i = 0; i < signerCount; ) {
            if (transactionConfirmations[signers[i]]) count += 1;
    ...
    function executeTransaction(
        ...
        if (isConfirmed(transactionId)) {
            ...
            if (txn.ts == TokenStandard.USER) {
                ...
                (bool transferSuccess, ) = txn.destination.call{
                    value: txn.value
                }(txn.data);
                if (transferSuccess) emit Execution(transactionId);
                else {
                ...
                IERC721(txn.token).safeTransferFrom(
                    address(this),
                    txn.destination,
                    txn.tokenId
                );
                ...
                IERC1155(txn.token).safeTransferFrom(
                    address(this),
                    txn.destination,
                    txn.tokenId,
                    txn.value,
    ...
    function isTransactionTimedOut(
        ...
        if (
            _getNow() <
            transaction.txTimestamp + transaction.confirmTimestamp ||
            transaction.confirmTimestamp == 0
        ) {
            return false;
        }
        return true;
    ...
    function isSignerChangeConfirmed(
        ...
        uint256 count;
        mapping(address => bool)
            storage confirmation = signerChangeConfirmations[_newSigner];
